...
beforeEach(() => {
    wrapper = shallow(<SubscriptionCard
    subscriptionAPI="https://example.com/"
    adobeCampaignServiceName="serviceName"
    subscriptionCountry="en"
    articleTitle="title"
    articleDescription="description"
    consentNoticeText="notice" />);
});
...
it('blurs the target when we press escape on the .card_name input', () => {
    input.simulate('keydown', { target: { blur: blurSpy }, keyCode: Keys.escape });
...
it('calls submit when we press enter on the .card_name input', () => {
    input.simulate('keydown', { target: { blur: blurSpy }, keyCode: Keys.enter });
